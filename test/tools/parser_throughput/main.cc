...
int main(int argc, char** argv)
{
  ...
  {
#ifndef BUILD_CSV
    THROW("CSV parser not enabled. Please reconfigure cmake and rebuild with BUILD_CSV=ON");
#endif
  ...
  else
  {
    ...
    VW::multi_ex examples;
    examples.push_back(&VW::get_unused_example(vw));
    while (VW::parsers::csv::parse_examples(vw, file_contents_as_io_buf, examples) != 0)
    {
